...
launched via `tmux-agent-workbench agent start --managed`, which stamps it with a `@workbench_task` marker. You
...
`tmux-agent-workbench prune` removes missing-root inspection windows without
live tasks or agent panes; `prune --dead-tasks` also clears retained task output.
